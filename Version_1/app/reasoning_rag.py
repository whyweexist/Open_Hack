...
2. Route through the MoE gating network → select top-K experts.
3. Each activated expert retrieves domain-specific chunks.
4. Build a structured reasoning prompt with the retrieved context.
5. Send to quantized LLM for chain-of-thought analysis.
6. Parse and validate the JSON output.
7. If LLM is unavailable / fails → fall back to an embedding-based
...
from app.config import CCPA_PDF_PATH, SIMILARITY_THRESHOLD, TOP_K_RETRIEVAL
...
class ReasoningRAGPipeline:
    ...
    def initialise(self) -> None:
        ...
        self._ready = True
        logger.info(
            "Pipeline ready — %d chunks, %d experts, LLM=%s",
            len(chunks),
            len(self.experts),
    ...
    def analyze(self, prompt: str) -> AnalyzeResponse:
        ...
        # Step 1 — MoE routing → expert opinions
        ...
        # Step 2 — Build context from expert-retrieved chunks
        context = self._build_context(expert_opinions)

        # Step 3 — LLM reasoning (with fallback)
        result = None
        ...
        return self._heuristic_classify(prompt, query_embedding, expert_opinions)

    # ── Context building ───────────────────────────────────────────────
    def _build_context(self, opinions: List[ExpertOpinion]) -> str:
        """Merge deduplicated retrieved chunks into a prompt context string."""
        ...
        return "\n\n---\n\n".join(context_parts[:10])  # cap at 10 chunks
    ...
    def _heuristic_classify(
        ...
        known violation / compliance patterns.
        """
        # Pattern matching
        ...
        if best_pattern["harmful"] and best_score > 0.45:
            # Merge pattern sections + expert sections
            sections = set(best_pattern["sections"]) | expert_sections
            return AnalyzeResponse(
                harmful=True,
        ...
        # Edge case: expert signals strong violation but pattern is unsure
        if expert_harmful_score > 0.5 and expert_sections:
            return AnalyzeResponse(
                harmful=True,
                articles=[f"Section {s}" for s in sorted(expert_sections)],
            )
